...
class Utilities:
    ...
    def sanity_check(self, sentence, block_size):
        ...
        input_tensor = torch.tensor(padded_sentence, dtype=torch.long, device=torch.device("cuda:0")).unsqueeze(0)

        # Display input tensor shape
        print("Input tensor shape:", input_tensor.shape)

        # Process the input tensor through the encoder model
        _, _,  attn_maps, _ = self.model(input_tensor) # Ignore the output of the model, and only get the attention maps; make sure your encoder returns the attention maps
        ...
        for j, attn_map in enumerate(attn_maps):
            ...
            fig, ax = plt.subplots()
            cax = ax.imshow(attn_map[0][0].cpu().numpy(), cmap='hot', interpolation='nearest')
            ax.xaxis.tick_top()  
            fig.colorbar(cax, ax=ax)  
            plt.title(f"Attention Map {j + 1}")
            
            # Save the plot
            plt.savefig(f"attention2_map_{j + 1}.png")
